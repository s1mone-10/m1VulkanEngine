...
namespace m1
{
	...
	public:
		 ...
		 */
		explicit
		TransformComponent(const std::string& componentName = "TransformComponent") : Component(componentName) {}

		/**
		 * @brief Constructor with matrix and an optional name.
		* @param matrix The matrix of the component.
		* @param componentName The name of the component.
		 */
		explicit
		TransformComponent(const glm::mat4& matrix, const std::string& componentName = "TransformComponent") : Component(componentName), matrix(matrix)
		{
			// TODO position, rotation and scale are not congruent with the provided matrix
			matrixDirty = false;
		}

		[[nodiscard]] size_t GetTypeID() const override { return Component::GetTypeID<TransformComponent>(); }
		...
		/**
		 * @brief Get the matrix for this transform.
	...
	private:
		/**
		 * @brief Update the matrix based on position, rotation, and scale.
		 */
		void UpdateMatrix();
	};
}
